          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', flex: 1 }}>
            {selectedMembers.map(m => (
              <span key={m.tm_id} className="lp-member-chip">
                <span className="lp-member-avatar-sm">
            ...
            {TEAM_MEMBERS.length === 0
              ...
              : TEAM_MEMBERS.map(m => {
                ...
                return (
                  ...
                  >
                    <span className="lp-member-avatar">
                      {(m.name || m.tm_name || '?')[0].toUpperCase()}
                    </span>
...
/* ─── Members display in task-list modal sidebar ─── */
...
export default function LeadProjects() {
  ...
  // Enrich PROJECTS with a members array derived from P_TEAMS
  const [projects, setProjects] = useState(() =>
    PROJECTS.map(p => ({
      ...p,
      members: P_TEAMS
        .filter(pt => pt.p_id === p.p_id)
        .map(pt => pt.tm_id),
    }))
  );
  ...
  const [editMembersFor,  setEditMembersFor]  = useState(null); // project p_id for inline member editing
  ...
  const filtered = projects.filter(p => {
    const s = p.p_name.toLowerCase().includes(search.toLowerCase()) ||
  ...
  /* update members inline (from task list modal) */
  const saveProjectMembers = (p_id, members) => {
    setProjects(prev => prev.map(p => p.p_id === p_id ? { ...p, members } : p));
    // also update taskListModal ref so header reflects change
  ...
  /* get member objects for a project */
  const getProjectMembers = (project) =>
    TEAM_MEMBERS.filter(m => (project.members || []).includes(m.tm_id));

  /* get member objects from id array */
  ...
  return (
    <div>
          ...
          : (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:16 }}>
              {filtered.map(project => {
                ...
                return (
                  <div key={project.p_id} className="card animate-fade-in"
      ...
      {taskListModal && !selectedTask && !editTask && (
        <div className="lp-overlay" onClick={() => { setTaskListModal(null); setEditMembersFor(null); }}>
          <div className="lp-modal lp-modal-wide" onClick={e => e.stopPropagation()}>
            ...
            {/* two-column: tasks + members sidebar */}
                  ...
                  : tasks.filter(t => t.p_id === taskListModal.p_id && !t.is_deleted).map(t => (
                    <div key={t.t_id} className="lp-task-row" onClick={() => setSelectedTask(t)}>
                      <div className="lp-task-info">
                        ...
                        {t.milestone && (
                          <span className="lp-milestone-tag"><Milestone size={10}/>{t.milestone}</span>
                        )}
                      ...
                      <div className="lp-task-meta">
                        ...
                        <div className="lp-mini-progress">
                          <div className="lp-mini-track">
                            <div className="lp-mini-fill" style={{ width:`${t.progress}%` }}/>
                          </div>
                      ...
                      <div className="lp-task-actions" onClick={e => e.stopPropagation()}>
                        ...
                        <button className="lp-icon-btn danger" title="Delete"
                          onClick={() => setDeleteTaskId(t.t_id)}>
                          <Trash2 size={12}/>
                        </button>
                      </div>
                    </div>
                ...
                {editMembersFor === taskListModal.p_id ? (
                  <div>
                    ...
                    <MemberPicker
                      ...
                      onChange={(newMembers) => setTaskListModal(prev => ({ ...prev, members: newMembers }))}
                      label=""
                    />
                    <div style={{ display:'flex', gap:8, marginTop:12 }}>
                      <button className="btn btn-primary btn-sm" style={{ flex:1 }}
                        onClick={() => saveProjectMembers(taskListModal.p_id, taskListModal.members || [])}>
                        Save
                      </button>
                      <button className="btn btn-secondary btn-sm" onClick={() => setEditMembersFor(null)}>
                        Cancel
                      </button>
                    </div>
                  </div>
          ...
          <div className="lp-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="lp-modal-body">
              ...
              {selectedTask.milestone && (
                <div className="lp-milestone-tag lg"><Milestone size={12}/>{selectedTask.milestone}</div>
              )}
        ...
        <div className="lp-overlay" onClick={() => setEditTask(null)}>
          <div className="lp-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="lp-modal-body lp-edit-form">
              ...
              <input className="input lp-input"
                value={editTask.title}
                onChange={e => setEditTask({ ...editTask, title: e.target.value })}/>

              <label className="lp-label">Description</label>
              <textarea className="input lp-input lp-textarea"
                value={editTask.desc || ''}
              ...
              <div className="lp-form-2col">
                <div>
                  ...
                  <select className="input lp-input"
              ...
              <input type="range" min={0} max={100} className="lp-range"
                value={editTask.progress}
                onChange={e => setEditTask({ ...editTask, progress: Number(e.target.value) })}/>
            </div>

            <div className="lp-modal-footer">
              <button className="btn btn-primary btn-sm" onClick={() => handleUpdateTask(editTask)}>
                Save Changes
              </button>
          ...
          <div className="lp-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="lp-modal-body lp-edit-form">
              ...
              <MemberPicker
                ...
                onChange={ids => setTaskForm({ ...taskForm, assignedTo: ids })}
        ...
        <div className="lp-overlay" onClick={() => setDeleteTaskId(null)}>
          <div className="lp-modal lp-confirm" onClick={e => e.stopPropagation()}>
            ...
            <div className="lp-modal-footer" style={{ justifyContent:'center' }}>
              <button className="btn btn-danger btn-sm" onClick={() => handleDeleteTask(deleteTaskId)}>
                Yes, Delete
              </button>
              <button className="btn btn-secondary btn-sm" onClick={() => setDeleteTaskId(null)}>
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="lp-overlay" onClick={() => setDeleteProjectId(null)}>
          <div className="lp-modal lp-confirm" onClick={e => e.stopPropagation()}>
            ...
            <p style={{ color:'var(--text-muted)', marginBottom:20, fontSize:13 }}>
              All tasks in this project will also be removed.
            </p>
            <div className="lp-modal-footer" style={{ justifyContent:'center' }}>
            ...
            <div className="lp-modal-body lp-edit-form">
              ...
              <div className="lp-form-2col">
                <div>
                  ...
                  <input type="date" className="input lp-input"
      ...
      <style>{`
        ...
        /* split layout: tasks + members sidebar */
        .lp-modal-body-split {
          display: grid;
        ...
        .lp-member-option:hover {
          background: var(--bg-secondary);
        }

        .lp-member-option.selected {
        ...
        .lp-chip-remove:hover {
          color: #ef4444;
          background: rgba(239,68,68,0.1);
        }

        /* ── Members sidebar in task list modal ── */
        .lp-members-sidebar {
          padding: 16px;
          overflow-y: auto;
          background: var(--bg-secondary);
        ...
        @media (max-width: 480px) {
          ...
          .lp-card-badges > * { max-width: 90px !important; font-size: 10px !important; padding: 2px 6px !important; }
        }
      `}</style>
    </div>
  );
